...
- `/squad` — Analyzes the project and generates domain-specific rules and specialist subagents. Breaks a codebase into 3-8 logical domains so Claude automatically loads the right context for each part. Use `/squad refresh` to update after structural changes.
...
- `/bosskey` — Summarizes recent git activity into a standup script. Reads actual diffs (not just commit messages) and translates implementation details into natural-sounding status updates.
